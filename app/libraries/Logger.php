...
class Logger {
  ...
  public function log() {
    $data        = array(
      ...
      $this->table_fields['ip_address'] => $this->ip_address,
  ...
  protected function _getQueryMaker() {
    ...
    if ($this->ip_address)
      $this->ci->db->where($this->table_fields['ip_address'], $this->ip_address);
    if ($this->token)
  ...
  public function get_ids() {
    $this->_getQueryMaker();
    $ids = $this->ci->db->select('ip_address')->get($this->tableName)->result_array();
    return array_column($ids, 'ip_address');
  }
